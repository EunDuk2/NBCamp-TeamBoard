...
class MainViewController: UIViewController {
    ...
    private func bindViewModel() {
        ...
        viewModel.didSelectAddTeam = { [weak self] in
            let addTeamVC = AddTeamViewController()
    ...
    private func updateSnapshot() {
        var newSanpshot = NSDiffableDataSourceSnapshot<Section, CellItem>()
        
        if viewModel.teams.isEmpty {
            newSanpshot.appendSections([.team])
            newSanpshot.appendItems([.addTeam], toSection: .team)
        } else {
            newSanpshot.appendSections(Section.allCases)
            
            let teamItems = viewModel.teams.map { CellItem.team($0) }
            newSanpshot.appendItems(teamItems, toSection: .team)
            
            let memberItems = viewModel.members.map { CellItem.member($0) }
            newSanpshot.appendItems(memberItems, toSection: .members)
            newSanpshot.appendItems([.addMember], toSection: .members)
        }
        
        datasource.apply(newSanpshot, animatingDifferences: false)
    }
